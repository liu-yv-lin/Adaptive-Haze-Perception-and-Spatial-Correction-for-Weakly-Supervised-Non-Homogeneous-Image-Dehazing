...
class SWRCAB_and_Up(nn.Module):
    def __init__(self, conv, in_feats, blocks):
        super(SWRCAB_and_Up, self).__init__()
        SWRCAB_blocks = [
            SWRCAB(in_feats,in_feats)for _ in range(blocks)
        ]
        self.up_blocks = nn.Sequential(*SWRCAB_blocks)
        self.up = Upsampler(conv, 2, in_feats)
